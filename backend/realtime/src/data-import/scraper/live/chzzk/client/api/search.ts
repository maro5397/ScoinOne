...
export class ChzzkSearch {
  ...
  async videos(keyword: string, options: SearchOptions = DEFAULT_SEARCH_OPTIONS): Promise<VideoSearchResult> {
  ...
  async lives(keyword: string, options: SearchOptions = DEFAULT_SEARCH_OPTIONS): Promise<LiveSearchResult> {
  ...
  async channels(keyword: string, options: SearchOptions = DEFAULT_SEARCH_OPTIONS): Promise<ChannelSearchResult> {
  ...
  async autoComplete(keyword: string, options: SearchOptions = DEFAULT_SEARCH_OPTIONS): Promise<string[]> {
    ...
    return this.client
      .fetch(`${this.client.options.baseUrls.gameBaseUrl}/v2/search/lounges/auto-complete?${params}`)
      .then((r) => r.json())
  ...
  async lounges(
    ...
    return this.client
      .fetch(`${this.client.options.baseUrls.gameBaseUrl}/v2/search/lounges?${params}`)
      .then((r) => r.json())
      .then((data) => data['content']);
  }

  async categories(keyword: string, options: SearchOptions = DEFAULT_SEARCH_OPTIONS): Promise<CategorySearchResult> {
    const params = new URLSearchParams({
      keyword,
      size: options.size.toString(),
      offset: options.offset.toString(),
    ...
    return this.client
      .fetch(`/manage/v1/auto-complete/categories?${params}`)
      .then((r) => r.json())
      .then((data) => data['content']);
  }
  ...
  private async search(
    ...
    return this.client
      .fetch(`/service/v1/search/${type}?${params}`)
      .then((r) => r.json())
      .then((data) => {
