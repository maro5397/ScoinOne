...
export async function accessToken(client: ChzzkClient, chatChannelId: string) {
  const r = await client.fetch(
    `${client.options.baseUrls.gameBaseUrl}/v1/chats/access-token?channelId=${chatChannelId}&chatType=STREAMING`,
  );
...
export async function profileCard(client: ChzzkClient, chatChannelId: string, userIdHash: string) {
  const r = await client.fetch(
    `${client.options.baseUrls.gameBaseUrl}/v1/chats/${chatChannelId}/users/${userIdHash}/profile-card?chatType=STREAMING`,
  );
  const data = await r.json();
  return data['content'] ?? null;
}
